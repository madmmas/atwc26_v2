locals {
  ...
  cloudfront_aliases = var.enable_custom_domain ? distinct(concat([var.domain_name], var.domain_subject_alternative_names)) : var.aliases

  cloudfront_certificate_arn = var.enable_custom_domain ? module.acm_certificate[0].certificate_arn : var.acm_certificate_arn
}
...
check "custom_domain_certificate" {
  assert {
    condition     = !var.enable_custom_domain || length(local.cloudfront_aliases) > 0
    error_message = "enable_custom_domain requires domain_name and/or domain_subject_alternative_names."
  }
}

check "external_certificate" {
  assert {
    condition     = var.enable_custom_domain || length(local.cloudfront_aliases) == 0 || var.acm_certificate_arn != null
    error_message = "When enable_custom_domain=false and aliases are set, acm_certificate_arn is required."
  }
}
