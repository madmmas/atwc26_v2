locals {
  function_name = "${var.name_prefix}-${var.environment}-analytics"
...
data "archive_file" "placeholder" {
  ...
  source {
    ...
    filename = "analytics_api/handler.py"
  }
}
...
resource "aws_lambda_function" "analytics" {
  ...
  handler       = "analytics_api.handler.handler"
  runtime       = "python3.11"
  architectures = ["arm64"]
  ...
  tags = merge(var.tags, { Component = "lambda-analytics" })
}
